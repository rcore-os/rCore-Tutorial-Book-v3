...
- **空闲则入** (Progress)：若资源处于空闲状态且有若干线程尝试进入临界区，那么只有这些线程能够决定最终其中哪个线程进入临界区，且一定能够在有限时间内选出哪个线程进入临界区。如果不满足空闲则入的话，可能导致即使资源空闲也没有线程能够进入临界区，对于锁来说是不可接受的。
...
.. 关于 peterson 算法的评价：精巧构造，多标记组合，不是单方面而需要双方配合
.. peterson 算法是否满足 progress 和 bounded waiting?
.. peterson 算法的拓展
.. 开始介绍内存一致性模型！
