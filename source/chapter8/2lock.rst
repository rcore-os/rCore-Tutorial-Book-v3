...
Rust 核心库在 ``core::sync::atomic`` 中提供了很多原子类型，比如我们这里可以使用 ``usize`` 对应的原子类型 ``AtomicUsize`` ，它支持很多原子操作。比如，第 12 行 ``fetch_add`` 单次操作就能将 ``A`` 的值加一并返回 ``A`` 之前的值。原子操作由核心库和硬件共同保证其 **原子性** (Atomicity)，含义是该操作的一系列功能要么全部完成，要么都不完成，而不会出现有些完成有些未完成的情况。之所以被称为“原子性”，正是为了强调其不可分的属性。如果操作的原子性可以保证，那么我们将不会在任何时候看到资源处于不合法的中间状态。

.. ISA提供的指令数量有限->原子操作范围有限
.. 原子性相关 note
