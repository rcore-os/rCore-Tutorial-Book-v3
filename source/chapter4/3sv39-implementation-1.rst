...
.. note::
    ...
    **大页** (Huge Page)
    ...
    所谓大页就是某些页的大小（如 :math:`2\text{MiB}` , :math:`1\text{GiB}`  ）大于常规缺省的页大小（如 :math:`4\text{KiB}` ）。本教程中并没有用到大页的知识，这里只是作为拓展，不感兴趣的读者可以跳过。
...
.. note::
    ...
    **分析 SV39 多级页表的内存占用**
    ...
    :math:`2\text{MiB}` 才会新分配一个 :math:`4\text{Kib}` 的第三级页表，而后者（对应根页表）每连续映射 :math:`1\text{GiB}` 才会新分配一个 :math:`4\text{Kib}` 的第二级页表。由于后者远小于前者，
...
上面主要是对单个应用的多级页表进行了介绍。在一个多任务系统中，可能同时存在多个任务处于运行/就绪状态，它们的多级页表
