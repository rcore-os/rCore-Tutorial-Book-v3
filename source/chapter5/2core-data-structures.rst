...
- 在运行过程中可能发生变化的元数据：则放在 ``TaskControlBlockInner`` 中，将它再包裹上一层互斥锁 ``Mutex<T>`` 放在任务控制块中。这是因为在我们的设计中外层只能获取任务控制块的不可变引用，若想修改里面的部分内容的话这需要 ``Mutex<T>`` 所提供的内部可变性。另外，当后续真正可能有多核同时修改同一个任务控制块中的内容时， ``Mutex<T>`` 可以提供互斥从而避免数据竞争。
