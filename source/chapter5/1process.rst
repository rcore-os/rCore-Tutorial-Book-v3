...
我们刚刚介绍了 ``fork/waitpid/exec`` 三个重要系统调用，借助它们我们可以开发功能更为强大的应用程序。下面我们通过描述两个重要的应用程序： **用户初始程序-init** 和 **外壳程序-user_shell** 的开发过程，来展示这些重要系统调用的使用方法。
...
- 第 19 行为 ``fork`` 返回值为 0 的分支，表示子进程，此行直接通过 ``exec`` 执行外壳程序 ``user_shell`` ，注意我们需要在字符串末尾手动加入 ``\0`` ，因为 Rust 在将这些字符串连接到只读数据段的时候不会插入 ``\0`` 。
...
由于外壳程序-user_shell需要捕获我们的输入并进行解析处理，我们需要加入一个新的用于输入的系统调用：
...
接下来就可以介绍外壳程序- ``user_shell`` 是如何实现的了：
