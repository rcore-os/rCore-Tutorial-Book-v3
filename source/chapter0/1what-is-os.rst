...
如果再简单一点，多站在应用程序的角度来看，我们可以发现现在的应用程序其实是运行在一个由硬件，操作系统，运行时库，图形界面所包起来的一个执行环境中。所以Android应用运行在ARM处理器上的Android操作系统中，微软的Office应用运行在x86-64处理器上的Windows操作系统中，Web Server应用运行在x86-64处理器上的Linux操作系统中。

所以，从应用程序的角度看，可以简单地把操作系统定义为：应用程序的软件执行环境。我们要学习甚至设计实现的一个简单操作系统，即使只支持运行一个显示“Hello World”的小应用软件。
