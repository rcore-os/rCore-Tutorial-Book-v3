...
这里我们把控制流在某一时刻的执行环境的状态（即确保下一时刻能继续正确执行控制流指令所需的硬件（主要是各种寄存器）内容）为 ``上下文 (context)`` 。对于异常控制流的 ``上下文 (context)`` 保存与恢复，主要是通过CPU和操作系统（手动编写在 ``stack`` 上保存与恢复寄存器的指令）来协同完成；对于函数转移控制流的 ``上下文 (context)`` 保存与恢复，主要是通过编译器（自动生成在 ``stack`` 上保存与恢复寄存器的指令）来帮助完成的。
