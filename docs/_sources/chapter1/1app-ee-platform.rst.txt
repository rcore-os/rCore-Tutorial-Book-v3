...
本节介绍了如何设计实现一个提供显示字符服务的用户态执行环境和裸机执行环境，以支持一个应用程序显示字符串。
在某种程度上看，执行环境的软件主体就可称为是一种操作系统。一个有显示字符能力的操作系统自身就具有了基本的调试能力。
本节进一步说明了不管执行环境是简单还是复杂，设计实现上是否容易，它都体现了不同操作系统的共性特征--给应用需求提供服务。
同时，操作系统向下直接或间接与硬件关联，向上可通过系统级的函数库间接给应用提供服务，
甚至可把系统函数库作为操作系统的一部分，来直接给应用提供服务。
