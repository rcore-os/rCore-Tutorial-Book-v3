...
**中断** (Interrupt) 和我们第二章中介绍的 程序错误导致或用于系统调用的 **陷入**  和 ``Trap`` 一样都可看作是某种异常 ，但是它们被触发的原因确是不同的。对于某个处理器核而言， 陷入与发起陷入的指令执行是 **同步** (Synchronous) 的，陷入被触发的原因一定能够追溯到某条指令的执行；而中断则 **异步** (Asynchronous) 于当前正在进行的指令，也就是说中断来自于哪个外设以及中断如何触发完全与处理器正在执行的当前指令无关。
