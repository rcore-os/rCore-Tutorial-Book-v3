...
特权级切换的起因
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
我们知道，批处理操作系统被设计为运行在 S 模式，这是由作为它运行环境的 SEE 所保证的；而应用程序被设计为运行在 U 模式，这个则是我们的批处理操作系统
所保证的。批处理操作系统作为应用程序的执行环境，需要在执行应用程序之前进行一些初始化工作，并监控应用程序的执行，具体体现在：
...
当从一般意义上讨论 RISC-V 架构的 Trap 机制时，通常需要注意两点：在
...
大多数的 Trap (陷入) 发生的场景都是在执行某条指令（如 ``ecall`` ）之后，CPU 发现触发了一个 Trap并需要进行处理。这条触发 Trap 的指令和进入 Trap 之前执行的最后一条指令不一定是同一条。比如代表上层软件的应用程序只是通过 ``ecall`` 指令
向代表底层执行环境的操作系统请求某项功能，那么上层软件期待底层环境处理完成后，还能够从上层软件被打断的位置继续执行，参考 :ref:`图示 <environment-call-flow>` 。
...
我们目前唯一的 CPU 正处在 U 特权级运行着一个执行流跑着应用程序的代码。在执行完某一条指令之后， CPU 发现一个
中断/异常被触发，于是必须将应用执行流暂停，先 Trap 到更高的 S 特权级去执行批处理操作系统提供的相应服务代码，等待执行
完了之后再回过头来恢复并继续执行应用执行流。

我们可以将 CPU 中间在 S 特权级执行的那一段也看成一个执行流，因为它全程只是以普通控制流的模式在 S 特权级执行。这个执行流的意义就在于
处理 Trap ，我们可以将其称之为 Trap 执行流，它在 Trap 触发的时候生成，并于 Trap 处理完毕之后消亡。于是，可以从从执行流的角度来看待 
Trap ： CPU 从应用程序执行流切换到 Trap 执行流，然后再切换回来继续运行。站在应用程序的角度， Trap 机制对它是完全透明的，无论在实际
执行的时候它在哪一条指令执行结束后进入 Trap ，它总是相信在 Trap 结束之后 CPU 能够以和被打断的时候相同的执行流状态来继续运行执行流，
就好像 Trap 从未发生过一样。
...
应用执行流被切换回来之后需要从暂停的位置恢复并继续执行，这需要在切换前后维持执行流状态的不变。执行流状态可以分为寄存器和栈两部分。
对于寄存器而言，每个 CPU 只有一套通用寄存器，在运行 Trap 
...
- 操作系统完成系统调用服务后，需要回复被打断的应用程序的Trap 上下文，并通 ``sret`` 让应用程序继续执行。
