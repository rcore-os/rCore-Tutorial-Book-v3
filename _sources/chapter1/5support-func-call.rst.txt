...
这里需要注意的是需要通过宏将 ``rust_main`` 标记为 ``#[unsafe(no_mangle)]`` 以避免编译器对它的名字进行混淆，不然在链接的时候， ``entry.asm`` 将找不到 ``main.rs`` 提供的外部符号 ``rust_main`` 从而导致链接失败。在 ``rust_main`` 函数的开场白中，我们将第一次在栈上分配栈帧并保存函数调用上下文，它也是内核运行全程中最底层的栈帧。
...
链接脚本中的 ``sbss``/``ebss`` 这类符号并不是 Rust 函数，它们只是链接器写入最终可执行文件的地址标记。为了在 Rust 代码中引用这些标记，我们把它们声明成 ``extern "C"`` 函数符号；但后续真正需要的是“这个符号所在的地址”，而不是调用这个函数。因此，旧代码常写成 ``sbss as usize``。在较新的 Rust 编译器中，直接把函数项转成整数地址会触发警告：编译器希望程序先明确地把符号看作一个指针，再从指针取得地址，这样可以区分“函数/符号本身”和“它在内存中的地址”这两个概念。

``linker_symbol_addr!`` 宏就是为统一处理这类转换而引入的。宏参数写成 ``$symbol:path``，是因为 ``path`` 片段可以匹配更多路径形式的符号，例如当前作用域中的 ``sbss``，也可以是通过模块路径访问的 ``crate::module::symbol``，而不局限于单个标识符。它先执行 ``$symbol as *const ()``，把外部符号转换成一个不关心具体类型的只读裸指针；随后调用指针的 ``addr()`` 方法取得它的整数地址。这样写的好处是：一方面避免在各处重复较长的转换表达式，另一方面也让代码意图更清楚——我们只是读取链接器符号的地址，并不打算调用它或解引用它。后续凡是需要取得链接脚本符号、汇编入口标号或函数入口地址的位置，都会复用这个宏。

需要注意的是，``linker_symbol_addr!`` 这种实现获取的是编译期（更准确地说，是链接期在链接器视角中看到的）符号地址，而我们在运行时真正想要的是变量或函数被加载到内存后的地址。这两个地址在概念上并不是一回事：如果可执行文件在加载时发生重定位，或者被加载到与链接脚本描述不同的位置，那么链接期地址和运行期地址就可能不同。目前我们对生成的二进制进行了裁剪，并且 QEMU 的加载方式会把内核放到链接脚本指定的位置，因此在本章的执行环境中可以保证这两个地址相同。

.. note::

    **Rust Tips：外部符号引用**

    ``extern "C"`` 可以引用一个外部的 C 函数接口（这意味着调用它的时候要遵从目标平台的 C 语言调用规范）。但我们这里只是引用位置标志，并通过 ``linker_symbol_addr!`` 宏将符号转成地址。由此可以知道 ``.bss`` 段两端的地址。

.. note::

    **Rust Tips：迭代器与闭包**

    ``clear_bss`` 中遍历地址区间的代码用到了 Rust 的迭代器与闭包的语法，它们在很多情况下能够提高开发效率。如读者感兴趣的话也可以将其改写为等价的 for 循环实现。
